...
plan_driven アプローチは、以下の4つのフェーズで構成されます：

### 1. Research（調査）
**目的**: 問題の理解と要件の明確化

**活動内容**:
- 課題要件（problem_statement.md）の詳細な読み込み
- 共通条件（README.md、CLAUDE.md）の確認
- 類似プロジェクトや既存実装の調査（必要に応じて）
- 技術的な制約や課題の洗い出し
...
**活動内容**:
- **DESIGN.md の作成**: アーキテクチャ、クラス設計、責任分離の明文化
- **plan.md の作成**: 実装タスクの細分化、各タスクの目的と完了定義の記述
- レイヤードアーキテクチャの設計（domain層、io層の分離）
- データフローの設計
- テスト戦略の策定（TDD/BDD アプローチの決定）
...
- **BDD の実践**:
  - 日本語テスト名で振る舞いを表現
  - Given-When-Then 構造のテストケース
  - 内側のループ（ユニットテスト）と外側のループ（E2Eテスト）
- **進捗管理**: plan.md でタスクの状態を記録

**成果物**:
- 実装コード（domain層、io層、メインエントリポイント）
- テストコード（ユニットテスト、E2Eテスト）
- Git コミット履歴（各TDDサイクルの記録）

### 4. Validate（検証）
**目的**: 実装の品質と要件適合性の確認

**活動内容**:
- **自動テストの実行**: 全テストがパスすることを確認
- **手動テストの実行**: 問題文の例を使った動作確認
- **コードレビュー**: 型ヒント、docstring、コメントの確認
- **ドキュメントの作成**: README.md の作成、プロジェクトルート README.md の更新
- **進捗の最終確認**: plan.md の完了状態の確認
...
このワークフローにより、**計画的かつ品質の高い実装** が実現できます。
