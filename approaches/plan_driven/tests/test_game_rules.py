...
def test_初期配置で黒番の合法手4つを正しく列挙する():
    """
    リバーシの初期配置では、黒番は4つの合法手を持つ。
    find_all_legal_moves はこれらを正しく列挙する。
    """
    # Given: リバーシの初期配置
    #    0 1 2 3 4 5 6 7
    # 0  . . . . . . . .
    ...
    # When: 黒番の全合法手を列挙
    合法手リスト = ルール.find_all_legal_moves('B')

    # Then: 4つの合法手が返される
    assert len(合法手リスト) == 4
    assert (2, 3) in 合法手リスト
    assert (3, 2) in 合法手リスト
    assert (4, 5) in 合法手リスト
    assert (5, 4) in 合法手リスト


def test_初期配置で白番の合法手4つを正しく列挙する():
    """
    リバーシの初期配置では、白番は4つの合法手を持つ。
    find_all_legal_moves はこれらを正しく列挙する。
    """
    # Given: リバーシの初期配置
    #    0 1 2 3 4 5 6 7
    ...
    # When: 白番の全合法手を列挙
    合法手リスト = ルール.find_all_legal_moves('W')

    # Then: 4つの合法手が返される
    assert len(合法手リスト) == 4
    assert (2, 4) in 合法手リスト
    assert (3, 5) in 合法手リスト
    assert (4, 2) in 合法手リスト
    assert (5, 3) in 合法手リスト


def test_合法手がない場合は空リストを返す():
    """
    どこにも合法手が存在しない場合、
    find_all_legal_moves は空リストを返す。
    """
    # Given: 合法手がない盤面（黒のコマしかない）
    #    0 1 2 3 4 5 6 7
    # 0  B B B B B B B B
    # 1  B B B B B B B B
    # 2  B B B B B B B B
